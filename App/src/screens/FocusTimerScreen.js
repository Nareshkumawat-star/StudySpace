...
import { View, Text, TouchableOpacity, StyleSheet, Animated as RNAnimated, Alert, ScrollView } from 'react-native';
...
const FocusTimerScreen = () => {
    ...
    // Timer states
    const [selectedPreset, setSelectedPreset] = useState(route.params?.duration || 25);
        ...
        <View style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <View style={[styles.header, { backgroundColor: colors.headerBg }]}>
                ...
                <View style={styles.headerTitleContainer}>
                    <Text style={[styles.headerTitle, { color: colors.text }]}>{getLocationText()}</Text>
                    {getSeatText() && (
                        <Text style={[styles.headerSubtitle, { color: colors.textSecondary }]}>{getSeatText()}</Text>
                    )}
                </View>
            ...
            >
                ...
                {!isBreak && !sessionStarted && (
                    <View style={styles.presetsContainer}>
                        <Text style={[styles.presetsLabel, { color: colors.textSecondary }]}>Select Duration</Text>
                                    ...
                                    style={[
                                        styles.presetButton,
                                        {
                                            backgroundColor: selectedPreset === preset.value 
                                                ? colors.primary 
                                                : colors.surface,
                                            borderColor: selectedPreset === preset.value 
                                                ? colors.primary 
                                                : colors.borderLight,
                                        },
                                    ]}
                ...
                <View style={[styles.liveIndicator, { backgroundColor: isBreak ? 'rgba(34, 197, 94, 0.1)' : colors.primaryLight }]}>
                    <View style={[styles.liveDot, { backgroundColor: isBreak ? colors.success : (sessionStarted && !isPaused ? colors.error : colors.primary) }]} />
                    <Text style={[styles.liveText, { color: isBreak ? colors.success : colors.primary }]}>
                        {isBreak ? 'Break Time' : (sessionStarted && !isPaused ? 'Live Session' : 'Ready')}
                    </Text>
                </View>
                ...
                {sessionStarted && (
                    <View style={styles.statsRow}>
                        <View style={[styles.statCard, { backgroundColor: colors.surface }]}>
                            ...
                            <Text style={[styles.statValue, { color: colors.text }]}>{completedBlocks}</Text>
                            <Text style={[styles.statLabel, { color: colors.textSecondary }]}>Blocks</Text>
                        </View>
                        <View style={[styles.statCard, { backgroundColor: colors.surface }]}>
                            <MaterialIcons name="schedule" size={20} color={colors.primary} />
                            <Text style={[styles.statValue, { color: colors.text }]}>{formatTotalTime(totalFocusTime)}</Text>
                            <Text style={[styles.statLabel, { color: colors.textSecondary }]}>Focus</Text>
                        </View>
                        <View style={[styles.statCard, { backgroundColor: colors.surface }]}>
                            <MaterialIcons name="stars" size={20} color="#a855f7" />
                            <Text style={[styles.statValue, { color: colors.text }]}>{pointsEarned}</Text>
                ...
                <View style={styles.avatarsRow}>
                    {activeUsers > 0 && [...Array(Math.min(3, activeUsers))].map((_, i) => (
                        <View key={i} style={[styles.avatarContainer, { borderColor: colors.surface, backgroundColor: colors.surfaceSecondary }]}>
                ...
                <Text style={[styles.studentsText, { color: colors.textSecondary }]}>
                    {activeUsers} {activeUsers === 1 ? 'student is' : 'students are'} focusing now
                </Text>
                ...
                <View style={[styles.progressCard, { backgroundColor: colors.surface }]}>
                    <View style={styles.progressHeader}>
                        <View>
                            ...
                            <Text style={[styles.progressValue, { color: colors.text }]}>{completedBlocks} Focus {completedBlocks === 1 ? 'Block' : 'Blocks'} Completed</Text>
                        </View>
                    ...
                    <View style={[styles.progressBarBg, { backgroundColor: colors.surfaceSecondary }]}>
                        <View style={[styles.progressBarFill, { width: `${Math.min(100, (completedBlocks / 4) * 100)}%`, backgroundColor: colors.primary }]} />
                    </View>
                ...
                <View style={styles.controls}>
                    ...
                    >
                        <MaterialIcons 
                            name={!sessionStarted ? "play-arrow" : (isPaused ? "play-arrow" : "pause")} 
                            size={24} 
                            color="white" 
                            style={{ marginRight: 8 }} 
                        />
                    ...
                    <View style={styles.secondaryControls}>
                        {isBreak && (
                            ...
                            >
                                ...
                                <Text style={[styles.secondaryButtonText, { color: colors.textSecondary }]}>Skip Break</Text>
                            </TouchableOpacity>
                        )}
                        ...
                        >
                            ...
                            <Text style={[styles.secondaryButtonText, { color: colors.textSecondary }]}>Leave</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </ScrollView>
